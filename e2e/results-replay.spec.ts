...
test('restores live scaffold when returning to "— Live —" after a replay', async ({ page, request }) => {
    ...
    // General Roman scaffold which has 6 sourceDataChecks (buildUniversalSourceDataChecks('roman')
    // in index.js). After replay, currentSelectedCalendar is clobbered to 'IT' and the scaffold
    // shows only 1 check. Returning to "— Live —" must re-sync state from the DOM controls and
    // rebuild the scaffold via setupPage(), restoring the 6-check General Roman layout.
    const run = {
        schemaVersion: 1,
